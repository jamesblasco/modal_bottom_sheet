...
class __SheetRouteContainerState extends State<_SheetRouteContainer>
  ...
  void onSheetExtentUpdate() {
    if (_routeController.value != _sheetController.animation.value) {
      if (!_sheetController.position.preventingDrag &&
          route.shouldPreventPopForExtent(_sheetController.animation.value)) {
        preventPop();
        return;
      }
      if (!_routeController.isAnimating) {
        final animationValue = _sheetController.animation.value.mapDistance(
          fromLow: 0,
  ...
  void onRouteAnimationUpdate() {
    ...
    if (!_firstAnimation &&
      ...
      if (_routeController.status == AnimationStatus.forward) {
        final animationValue = _routeController.value.mapDistance(
          fromLow: 0,
          fromHigh: 1,
  ...
  Widget build(BuildContext context) {
    ...
    SheetPhysics? physics = SnapSheetPhysics(
      stops: route.stops ?? [0, 1],
      parent: route.physics,
    );
    if (!route.draggable) {
      physics = NeverDraggableSheetPhysics();
    }
    return Semantics(
      scopesRoute: true,
      namesRoute: true,
      ...
      child: Sheet.raw(
        ...
        decorationBuilder: route.decorationBuilder,
        fit: route.fit,
        physics: physics,
